...
void
RtoChange (std::ofstream* ofStream, Time oldRto, Time newRto)
{
  *ofStream << Simulator::Now().GetSeconds() <<'\t' << oldRto.GetSeconds() << '\t' << newRto.GetSeconds() <<std::endl;
}
...
double t_total[10];
void
TraceConnectionClient (std::ofstream* ofStream1, std::ofstream* ofStream2, std::ofstream* ofStream3, 
                      Ptr<const ThreeGppHttpClient> client)
{
  ...
  //client->GetSocket()->TraceConnectWithoutContext("RTO", MakeBoundCallback (&RtoChange, ofStream2));
}
...
main (int argc, char *argv[])
{
  ...
  std::string operationMode = "FDD";  // TDD or FDD (mixed TDD and FDD mode)
  ...
  uint32_t objectSizeAR = 1024*1024*10; 

  //uint32_t objectSizeAR = 1024*1024;
  ...
  //std::string folder_name = "lte_preliminary";
...
//      LogComponentEnable ("NrUeMac", LOG_LEVEL_INFO);
      ...
      for (uint32_t j = 1; j <= ueNumPergNb; ++j)
        {
          ...
          if (yValue > 0)
            {
              staPositionAlloc->Add (Vector (xValue, 70, ueHeight));
            }
          else
            {
              staPositionAlloc->Add (Vector (xValue, -70, ueHeight));
  ...
  CcBwpCreator::SimpleOperationBandConf bandConf (centralFrequencyBand38, bandwidth,
                                                  numCcs, BandwidthPartInfo::UMi_StreetCanyon_LoS);
  bandConf.m_numBwp = operationMode == "FDD" ? 2 : 1;
  ...
  */
  ...
  if (operationMode == "TDD")
    {
      ...
      nrHelper->GetGnbPhy (enbNetDev.Get (0), 0)->SetAttribute ("TxPower",
                                 DoubleValue (10 * log10 ((band.GetBwpAt (0, 0)->m_channelBandwidth/totalBandwidth) * x)));
      nrHelper->GetGnbPhy (enbNetDev.Get (0), 0)->SetAttribute ("Pattern", StringValue (pattern));
      nrHelper->GetGnbPhy (enbNetDev.Get (0), 0)->SetAttribute ("RbOverhead", DoubleValue (0.1));
    }
  else  //FDD case
    {
      nrHelper->GetGnbPhy (enbNetDev.Get (0), 0)->SetAttribute ("Numerology", UintegerValue (numerologyBwpDl));
      nrHelper->GetGnbPhy (enbNetDev.Get (0), 0)->SetAttribute ("TxPower",
                                 DoubleValue (10 * log10 ((band.GetBwpAt (0, 0)->m_channelBandwidth/totalBandwidth) * x)));
      nrHelper->GetGnbPhy (enbNetDev.Get (0), 0)->SetAttribute ("Pattern", StringValue (patternDL));
      nrHelper->GetGnbPhy (enbNetDev.Get (0), 0)->SetAttribute ("RbOverhead", DoubleValue (0.1));

      nrHelper->GetGnbPhy (enbNetDev.Get (0), 1)->SetAttribute ("Numerology", UintegerValue (numerologyBwpUl));
      nrHelper->GetGnbPhy (enbNetDev.Get (0), 1)->SetAttribute ("Pattern", StringValue (patternUL));
      nrHelper->GetGnbPhy (enbNetDev.Get (0), 1)->SetAttribute ("RbOverhead", DoubleValue (0.1));
  ...
  std::string rtoTraceFile = "rto.dat";
  std::ofstream rtoStream;
  rtoStream.open (rtoTraceFile.c_str(), std::ofstream::out);
  rtoStream<<"Time\tOld\tNew"<<std::endl;
  ...
  EpsBearer ARBearer (EpsBearer::NGBR_VOICE_VIDEO_GAMING);
